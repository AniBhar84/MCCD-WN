...
void pam(long double **man_dist,long int *ind,long int nnode,long int n_cm,long int *clustering,long double *bwsc);
long int pam_swap(long double **man_dist,long int n_cm,long int nnode,long int *ind,long int *clustering, long double *cost);
...
int main(int argc,char *argv[])
{
	...
	//cl_data=(long double **)calloc(nnode,sizeof(long double *));
	//norm_cl_data=(long double **)calloc(nnode,sizeof(long double *));
	...
	for(n_cm=K1;n_cm<=K2;n_cm++)
	{
		...
		if(n_cm==2)
		{
			...
			//Q=dense_score(A,kval,n_cm,nnode,clustering);
			Qmax=Q;
			Qmax_ind=n_cm;
			//printf("\n Modularity is %LF",Q);
		...
		else
		{
			...
			printf("\n");*/
			//pam(man_dist,ind,nnode,n_cm,clustering,bwsc);
...
void pam(long double **man_dist,long int *ind,long int nnode,long int n_cm,long int *clustering, long double *bwsc)
{
	...
	long int i, j, min_ind, cnt1, flag=1, iter=1, *ind_old, k;
	long double min, obj,obj1, *cost;
	ind_old=(long int *)malloc(n_cm*sizeof(long int));
	cost=(long double *)calloc(n_cm,sizeof(long double));
	while(flag==1)
	{
		obj=0;
		for(i=0;i<nnode;i++)
		{
			...
			for(j=0;j<n_cm;j++)
			{
				...
				else
				{
					if(cnt1==0)
					{
						min=man_dist[i][ind[j]];
						min_ind=j;
						cnt1++;
					}
					if(cnt1>0)
					{
						if(man_dist[i][ind[j]]<min)
						{
							min=man_dist[i][ind[j]];
							min_ind=j;
							cnt1++;
			...
			cost[min_ind]=cost[min_ind]+man_dist[i][ind[min_ind]];
			clustering[i]=min_ind+1;
			obj=obj+man_dist[i][ind[min_ind]];
		...
		if(iter==1)
		{
			...
			flag=pam_swap(man_dist,n_cm,nnode,ind,clustering,cost);
			//flag=pam_swap_new(man_dist,n_cm,nnode,ind,clustering,bwsc);
		}
		else
		{
			if(obj<obj1)
...
long int pam_swap(long double **man_dist,long int n_cm,long int nnode,long int *ind,long int *clustering, long double *cost)
{
	...
	long int i, j, k, cnt1, min_dist_ind, flag=0, cent_ind_pos, cent_ind, fl, k1;
	long double dist_sum, min_dist_sum, diff_cost, max_diff_cost;
	...
	for(k=0;k<n_cm;k++)
	{
		...
		for(i=0;i<nnode;i++)
		{
			...
			if(fl==1)
			{
				...
				for(j=0;j<nnode;j++)
				{
					if(clustering[j]==(k+1))
					{
						dist_sum=dist_sum+man_dist[i][j];
					}
				}
				if(cnt1==1)
				{
		...
		if(ind[k]!=min_dist_ind)
		{
			//ind[k]=min_dist_ind;
			flag=1;
			diff_cost=cost[k]-min_dist_ind;
			if(k==0)
			{
				max_diff_cost=diff_cost;
				cent_ind_pos=k;
				cent_ind=min_dist_ind;
				
			}
			else
			{
				if(diff_cost>max_diff_cost)
				{
					max_diff_cost=diff_cost;
					cent_ind_pos=k;
					cent_ind=min_dist_ind;
